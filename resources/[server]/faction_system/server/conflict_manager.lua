...
FALA PESSOAL SEJAM BEM VINDOS
    ...
    Este script gerencia os conflitos entre facções no sistema. Ele permite iniciar e 
    concluir conflitos, bem como atualizar o banco de dados com informações relacionadas 
    a esses conflitos.

    Eventos e suas funções:

    1. `faction_system:startConflict`:
        - Inicia um novo conflito entre facções por um território específico.
        - Insere informações sobre o conflito no banco de dados.
        - Parâmetros:
    ...
    2. `faction_system:endConflict`:
        - Conclui um conflito registrado e atualiza o banco de dados com o vencedor e a hora de término.
        ...
        - O proprietário do território é atualizado para a facção vencedora.

    Observações:
    - Certifique-se de que as tabelas `conflicts` e `territories` existem no banco de dados com 
      os campos apropriados para armazenar informações de conflitos e territórios.
